...
int main(int argc, char* argv[]) {
  ...
  if (!in_str) {
    std::cerr << "Bad file name: " << argv[1] << std::endl;
    return 1;
  }
  ...
  while (in_str >> request) {
    ...
    if (request == "add_person") {
      ...
      if (friend_graph.remove_friendship(name1, name2)) 
        ...
        std::cerr << "Error trying to unlink friends " << name1 << " and " << name2 << std::endl;
    } 
    ...
    // adding and passing messages 
      ...
      if (friend_graph.add_message(name1,msg)) 
        ...
        std::cerr << "Error trying to add message " << msg << " to person " << name1 << std::endl;
    ...
    // print the invitation list
      ...
      if (!friend_graph.print_invite_list(std::cout,name1,count, members))
        std::cerr << "Error trying to print invitation list for person " << name1 << std::endl;
		else {
			//print out our set of members. Sets are automatically sorted, 
			//so the order will always be correct.
			std::cout << "Invite list of " << name1 << ' ' << count << ": ";
			for (std::set<std::string>::iterator mems = members.begin(); mems != members.end(); ++mems) 
				std::cout << *mems << ' ';
			std::cout << std::endl;
    ...
    // print a text version of the whole graph
      ...
      std::cerr << "Illegal input: " << request << std::endl;
    }
